...
CU_pSuite register_arm64_tests(void) {
    CU_pSuite suite;
    INIT_SUITE(suite);
    ADD_DISASM_TEST(suite, test_set_reg_simple);
    ADD_DISASM_TEST(suite, test_reg_multiple);
    ADD_DISASM_TEST(suite, test_reg_split);
    ADD_DISASM_TEST(suite, test_reg_neg);
    ADD_DISASM_TEST(suite, test_reg_neg_split);
    ADD_DISASM_TEST(suite, test_inc_dec_reg);
    ADD_DISASM_TEST(suite, test_load_store);
    ADD_DISASM_TEST(suite, test_add_sub_reg);
    ADD_DISASM_TEST(suite, test_add_sub_byte);
    ADD_DISASM_TEST(suite, test_set_byte);
    ADD_DISASM_TEST(suite, test_inc_dec_wrapper);
    ADD_DISASM_TEST(suite, test_reg_copy);
    ADD_DISASM_TEST(suite, test_syscall);
    ADD_DISASM_TEST(suite, test_jmp_padding);
