...
static inline bool x86TestJcc(char tttn, uint8_t reg, int32_t offset, int fd) {
    uint8_t i_bytes[9] = {
        /* TEST byte [reg], 0xff */
        0xf6, reg, 0xff,
...
bool eamAsmJumpNotZero(uint8_t reg, int32_t offset, int fd, off_t *sz) {
    *sz += 9;
    ...
    return x86TestJcc(0x5, reg, offset, fd);
}
...
bool eamAsmJumpZero(uint8_t reg, int32_t offset, int fd, off_t *sz) {
    *sz += 9;
    ...
    return x86TestJcc(0x4, reg, offset, fd);
}
...
static inline bool x64AddSubQW(uint8_t reg, int64_t imm64, int fd, uint8_t op) {
...
bool eamAsmAddRegQuadWord(uint8_t reg, int64_t imm64, int fd, off_t *sz) {
    *sz += 15;
    ...
    return x64AddSubQW(reg, imm64, fd, 0x00);
}
...
bool eamAsmSubRegQuadWord(uint8_t reg, int64_t imm64, int fd, off_t *sz) {
    *sz += 15;
    ...
    return x64AddSubQW(reg, imm64, fd, 0x28);
}
