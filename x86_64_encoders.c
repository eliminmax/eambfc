...
bool x86_64_reg_copy(uint8_t dst, uint8_t src, int fd, off_t *sz) {
    *sz += 2;
    return write_obj(fd, (uint8_t[]){ 0x89, 0xc0 + (src << 3) + dst}, 2);
}

/* SYSCALL */
bool x86_64_syscall(int fd, off_t *sz) {
    *sz += 2;
    return write_obj(fd, (uint8_t[]){ 0x0f, 0x05 }, 2);
}
...
bool x86_64_nop_loop_open(int fd, off_t *sz) {
    *sz += 9;
    ...
    return write_obj(fd, &nops, JUMP_SIZE);
}
...
static inline bool x86_offset(char op, uint8_t adm, uint8_t reg, int fd) {
    return write_obj(fd, (uint8_t[]){ 0xfe | (adm&1), (op|reg|(adm<<6)) }, 2);
}
...
bool x86_64_set_reg(uint8_t reg, int64_t imm, int fd, off_t *sz) {
    if (imm == 0) {
        *sz += 2;
        /* XOR reg, reg */
        return write_obj(fd, (uint8_t[]){ 0x31, 0xc0|(reg<<3)|reg }, 2);
    }

    if (imm >= INT8_MIN && imm <= INT8_MAX) {
        *sz += 3;
        /* PUSH imm8; POP reg */
        return write_obj(fd, (uint8_t[]){ 0x6a, imm, 0x58 + reg}, 3);
    }
...
bool x86_64_add_reg_imm8(uint8_t reg, int8_t imm8, int fd, off_t *sz) {
    *sz += 3;
    return write_obj(fd, (uint8_t[]){ 0x83, 0xc0 + reg, imm8 }, 3);
}

/* { */
bool x86_64_sub_reg_imm8(uint8_t reg, int8_t imm8, int fd, off_t *sz) {
    *sz += 3;
    return write_obj(fd, (uint8_t[]){ 0x83, 0xe8 + reg, imm8 }, 3);
}
...
static inline bool add_sub_qw(uint8_t reg, int64_t imm64, int fd, uint8_t op) {
    ...
    return write_obj(fd, &i_bytes, 15);
