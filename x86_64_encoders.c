...
/* TEST byte [reg], 0xff */
static inline \
    bool eamAsmJumpTest(uint8_t reg, int fd) {
    return write(fd, (uint8_t[]){ 0xf6, reg, 0xff}, 3) == 3;
}

/* TEST byte [reg], 0xff; Jcc|tttn jmp_offset */
static inline \
    bool eamAsmCondJump(char tttn, uint8_t reg, int32_t offset, int fd) {
...
static inline \
    bool eamAsmOffset(char op, uint8_t adm, uint8_t reg, int fd) {
...
/* MOV reg, imm32 */
static inline \
    bool eamAsmSetRegDoubleWord(uint8_t reg, int32_t imm32, int fd) {
...
static inline \
    bool eamAsmAddSubRegQW(uint8_t reg, int64_t imm64, int fd, uint8_t opcode) {
    ...
    uint8_t i_bytes[] = {
        ...
        /* (ADD||SUB) reg, tmp_reg */
        0x48, 0x01 + opcode, 0xc0 + (tmp_reg << 3) + reg,
        /* POP tmp_reg */
        0x58 + tmp_reg
    };
...
bool eamAsmAddRegQuadWord(uint8_t reg, int64_t imm64, int fd, off_t *sz) {
    *sz += 15;
    ...
    return eamAsmAddSubRegQW(reg, imm64, fd, 0x00);
}
...
bool eamAsmSubRegQuadWord(uint8_t reg, int64_t imm64, int fd, off_t *sz) {
    *sz += 15;
    ...
    return eamAsmAddSubRegQW(reg, imm64, fd, 0x28);
}
